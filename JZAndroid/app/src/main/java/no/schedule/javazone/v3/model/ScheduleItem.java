 ...
 */
package no.schedule.javazone.v3.model;
...
import no.schedule.javazone.v3.provider.ScheduleContract.Blocks;
...
public class ScheduleItem implements Cloneable, Comparable<ScheduleItem> {
    ...
    public void setTypeFromBlockType(String blockType) {
        if (Blocks.isValidBlockType(blockType) &&
                Blocks.BLOCK_TYPE_BREAK.equals(blockType)) {
            type = BREAK;
        } else {
            type = FREE;
        }
    ...
    public int hashCode() {
        return Objects.hash(type, sessionType, blockKind, mainTag, tags, startTime, endTime,
                ...
                hasGivenFeedback, backgroundImageUrl, backgroundColor, flags);
    }
}
