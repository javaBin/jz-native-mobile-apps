...
class UserCollectionViewCell: UICollectionViewCell, CellInterface {
    ...
    func bind(_ user: User) {
        ...
        nameListLabel.text = user.name + " " + user.surname
        nameGridLabel.text = nameListLabel.text
        let userPostsString = (String(user.postsCount) + " posts • ")
        let userCommentsString = (String(user.commentsCount) + " comments • ")
        let userLikesString = (String(user.likesCount) + " likes")
